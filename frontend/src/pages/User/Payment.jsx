...
const OrderForm = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        ...
        {
          amount: Math.round(totalPrice * 100), // ✅ FIXED
        },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      if (paymentIntent.status === "succeeded") {
        await axios.post(
          ...
          {
            ...payload,
            paymentResponse: paymentIntent, // ✅ CRITICAL FIX
          },
          {
            headers: { Authorization: `Bearer ${token}` },
          },
  ...
  return (
    <div className="lg:col-span-2 bg-[#080808] border border-white/5 p-8 shadow-2xl">
      <h2 className="text-[10px] tracking-[0.5em] uppercase font-bold text-[#A37E2C] mb-8">
        Shipping & Registry
      </h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input
            ...
            className="bg-transparent border-b border-white/10 p-3 focus:border-[#A37E2C] outline-none transition-colors text-sm text-white"
          />
          <input
            required
            name="lastname"
        ...
        <textarea
          ...
          onChange={handleChange}
          className="w-full bg-transparent border-b border-white/10 p-3 focus:border-[#A37E2C] outline-none transition-colors text-sm h-24 resize-none text-white"
        />

        <div className="pt-6">
          <p className="text-[9px] tracking-widest text-gray-500 uppercase font-bold mb-4">
            Payment Selection
          </p>
            ...
            {["card", "cod"].map((method) => (
              ...
              >
                <input
                  ...
                  className="accent-[#A37E2C]"
                />
                <span
                  className={`text-[10px] uppercase tracking-widest ${paymentMethod === method ? "text-white" : "text-gray-500"} group-hover:text-[#A37E2C] transition-colors`}
                >
                  {method === "card"
                    ? "Secure Card / Digital"
                    : "Cash on Delivery"}
        ...
        {paymentMethod === "card" && (
          <motion.div
            ...
            className="p-4 border border-white/5 bg-white/[0.02] mt-4"
          >
            <CardElement
              options={{
                style: {
        ...
        <button
          disabled={loading}
          className="w-full md:w-auto px-12 py-4 bg-[#A37E2C] text-black text-[10px] tracking-[0.4em] uppercase font-bold hover:bg-white transition-all duration-700 disabled:opacity-50 mt-8"
        >
  ...
  return (
    <div className="bg-[#050505] min-h-screen text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-32 pb-20">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-[10px] tracking-widest uppercase text-gray-500 hover:text-[#A37E2C] mb-12 transition-colors"
        >
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <aside className="space-y-8">
            <div className="bg-[#080808] border border-white/5 p-6 shadow-2xl sticky top-32">
              <h3 className="text-[10px] tracking-[0.4em] uppercase font-bold text-[#A37E2C] mb-6">
                Manifest Summary
              </h3>
                  ...
                  return (
                    <div key={idx} className="flex gap-4 group">
                      <div className="flex-1">
                        <p className="text-xs uppercase tracking-widest font-bold text-white/90 line-clamp-1">
                          {p.title || p.name}
                        </p>
                        <p className="text-[9px] text-gray-500 mt-1 uppercase tracking-tighter">
                          Qty: {p.quantity || 1}
                        </p>
                        <p className="text-sm font-serif italic text-[#A37E2C] mt-1">
                          {formatPrice(p.price)}
                        </p>
                      </div>
                    </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/5 space-y-3">
                ...
                <div className="flex justify-between text-xl font-serif italic pt-4 text-white border-t border-white/5 mt-2">
                  <span>Total</span>
                  <span>{formatPrice(subtotal + 40)}</span>
                </div>
              </div>
            </div>
          ...
          >
            ...
            >
              <div className="relative w-24 h-24 mx-auto">
                <CheckCircle2
                  size={96}
                  className="text-[#A37E2C] absolute inset-0 animate-pulse"
                  strokeWidth={1}
                />
              </div>
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-400 tracking-wide leading-relaxed">
                  Your selection has been successfully registered in the House
                  of Rajubhai. A concierge curator will reach out shortly via
                  phone to finalize delivery.
                </p>
              </div>
              <button
